...
async function mount() {
  ...
  <main class="main sc-main">
    ...
    <section class="sc-play" aria-label="${t("그림 추리")}"><div class="sc-picture-column">
      ...
      <form id="sc-form" autocomplete="off" ${item.outcome ? "hidden" : ""}><div class="sc-search"><label>${icon("search")}<input id="sc-input" type="search" role="combobox" aria-label="${t("포켓몬 이름 또는 도감 번호")}" aria-controls="sc-options" aria-autocomplete="list" aria-expanded="false" placeholder="${t("이름 또는 도감 번호")}" value="${esc(query)}" spellcheck="false" autocomplete="off" /></label><button id="sc-submit" class="primary-button" aria-label="${t("추측 제출")}">${icon("arrow-right")}</button></div><ul id="sc-options" role="listbox" aria-label="${t("검색 결과")}" hidden></ul></form>
